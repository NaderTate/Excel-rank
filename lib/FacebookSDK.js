...
export const getInstagramPageData = (pageId, pageToken) => {
  return new Promise(async (resolve, reject) => {
    const data = await fetch(
      `https://graph.facebook.com/v18.0/${pageId}?fields=followers_count,media{caption,comments_count,like_count,media_url,permalink,username,timestamp,media_type,children{media_url}},name,media_count,profile_picture_url,username,website,follows_count,biography,ig_id&access_token=${pageToken}`
    );
    const dataJson = await data.json();
    resolve(dataJson);
  });
