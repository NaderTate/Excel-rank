...
export const getPageData = (pageId, pageToken) => {
  return new Promise(async (resolve, reject) => {
    const data = await fetch(
      `https://graph.facebook.com/v18.0/${pageId}?fields=name,picture{url},published_posts{message,created_time,permalink_url},followers_count,access_token&access_token=${pageToken}`
    );
    const dataJson = await data.json();
