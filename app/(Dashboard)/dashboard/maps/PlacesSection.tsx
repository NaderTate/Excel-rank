...
export default function PlacesSection({ setPlace }: any) {
  ...
  const handleSelect = async (val: string, main_text: string) => {
    ...
    setPlace({ position: { lat, lng }, id: details.place_id, suggestedKeywords: details.types });
  };
  ...
  return (
    <div className="my-5 p-2 md:p-10 ">
      <h1 className="text-xl md:text-3xl my-2 font-bold">Find your buisness 🔎</h1>
      <div id="SearchBox" className="flex justify-center relative">
        ...
        <ul className="absolute top-12 w-full bg-white rounded-lg shadow-lg z-10 flex flex-col gap-1">
          {data.map((suggestion) => {
            ...
            return (
              <li key={place_id} onClick={() => handleSelect(place_id, main_text)} className="p-2 hover:bg-gray-200 cursor-pointer">
      ...
      <div className="flex flex-col gap-2 mt-10 bg-gray-300 shadow-sm border rounded-xl p-4">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="flex flex-col gap-2">
          <div onClick={() => setClosed(!closed)} className="flex justify-between items-center cursor-pointer">
            <h1 className="xs:text-lg sm:text-xl font-bold">How to find your business?</h1>
          ...
          <AnimatePresence>
            {!closed && (
              ...
              >
                ...
                <p className="sm:text-lg">
                  <span className="font-bold">Step 3:</span> Click on the next button
                </p>
              </motion.div>
            )}
          </AnimatePresence>
