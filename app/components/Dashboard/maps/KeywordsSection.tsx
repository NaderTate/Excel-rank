...
export default function KeywordsSection({
  ...
  return (
    <div className="my-5 p-2 md:p-10 ">
      ...
      <div
        onClick={focusInput}
        className="flex flex-wrap p-2 gap-2 border bg-background rounded-lg min-h-[50px]"
      >
        {keywords &&
          keywords.map((word, index) => (
            <div
              key={index + "keyword" + word}
              className=" p-1 flex items-center justify-center text-sm bg-gray-500/30 rounded-lg border"
            >
              <span>{word}</span>
      ...
      <div className="flex flex-col gap-2 mt-10 bg-background shadow-sm border rounded-xl p-4">
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
              ...
              >
                {suggestedKeywords.map((word) => (
                  <div
                    ...
                    className=" p-1 flex flex-1 items-center justify-between text-sm cursor-pointer  transition hover:bg-slate-200 dark:hover:bg-slate-800 rounded-lg border"
                  >
                    <span>{word}</span>
                    {/* Add */}
