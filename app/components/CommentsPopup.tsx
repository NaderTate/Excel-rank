...
function CommentsPopup({ postId, pageToken, commentsCount }: { postId: string; pageToken: string; commentsCount: number | null }) {
  const [comments, setComments] = useState<[] | null>(null);
  const [showComments, setShowComments] = useState<boolean>(false);

  const getComments = () => {
    if (showComments) return;
    fetch(`https://graph.facebook.com/v18.0/${postId}/comments?access_token=${pageToken}`)
      .then((res) => res.json())
      .then((res) => {
    ...
    <Dialog>
      ...
      <DialogContent className="min-h-[70vh] flex flex-col items-start w-full bg-[#18191a] backdrop-blur-sm border-0 rounded-xl text-gray-50">
        ...
        {comments?.map((comment: any) => (
          <div key={comment.id} className="border border-blue-300 rounded-md p-2 w-full">
            <div className="flex items-center">
              <h1 className="text-lg font-bold  mr-1">{comment.from?.name}</h1>
              <span className="text-sm text-gray-100">({new Date(comment.created_time).toDateString()})</span>
            </div>
