    ...
    >
      <div className="py-2 sm:py-1 px-2 md:px-5 lg:px-8 sm:p-8flex itmes-center ">
        <AnimatedLink href="/" className="flex items-center justify-center">
          <Image className=" w-8 h-8 lg:w-10 lg:h-10" src="/logo.svg" width={100} height={100} alt="logo" />
          <div className="hidden gradient_text sm:block ml-2 text-xl font-bold text-gray-900 dark:text-white sm:text-2xl md:text-3xl">Craft Care</div>
        </AnimatedLink>
      </div>
        ...
        {session?.user ? (
          <>
            <div className="group hidden md:flex shrink-0 items-center rounded-lg transition ">
              ...
              <img alt="Profile" src={session.user?.image} className="h-10 w-10 md:h-12 md:w-12 rounded-full object-cover" />
